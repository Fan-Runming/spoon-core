...
def translate_all():
    ...
    replacements = [
        ...
        # 按钮提示
        ('title="语音输入"', 'title="Voice input"'),
        ('title="上传照片"', 'title="Upload photo"'),
        ('发送', 'Send'),
        ...
        ('正在Upload photo…', 'Uploading photo…'),
        ('上传失败', 'Upload failed'),
        ('照片Upload failed。', 'Photo upload failed.'),
        ...
        # ⭐ 修复：搜索关键词（JavaScript字符串）
        ('t.startsWith("找")', 't.startsWith("find")'),
        ('t.startsWith("查找")', 't.startsWith("search")'),
    ]
    ...
    print(f'\n✅ 翻译完成！总共执行了 {len(replacements)} 次替换')
    print(f'📄 文件大小: {len(original_content)} → {len(content)} bytes')
